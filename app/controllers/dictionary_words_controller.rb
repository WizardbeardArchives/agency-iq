class DictionaryWordsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @dictionary_word.save
        ...
        format.json { render action: 'show', status: :created, location: @dictionary_word }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to dictionary_words_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def dictionary_word_params
      params.require(:dictionary_word).permit(:primaryKey, :DWORD_DICT_NAME, :DWORD_WORD)
    end
end
